...
export class UserController {
  ...
  async updateById(req: CustomRequest, res: Response) {
    try {
      const data = await userService.updateById(req.params.id, req.body, req.file?.path);
  ...
  async updateProfile(req: CustomRequest, res: Response) {
    try {
      const data = await userService.updateProfile(req.body, req.user, req.file?.path);
      if (req.file?.filename) removeFile(req.file.filename);
      return responseSuccessWithData(res, data);
    } catch (error: any) {
      console.log(error);
