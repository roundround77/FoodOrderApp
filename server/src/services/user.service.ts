...
import { base64_encode, pagination, removeFile } from './base.service';
...
class UserService {
  ...
  async updateById(id?: string, data?: IUser, file?: string) {
    ...
    if (file) avatar = base64_encode(file);

    console.log(file);
    console.log(avatar);

    const update = await this.userModel.findByIdAndUpdate(id, { ...data, avatar, updatedAt: new Date() }, { new: true });
  ...
  async updateProfile(data?: IUser, userAuth?: UserAuth, file?: string) {
    ...
    const update = await this.userModel.findByIdAndUpdate(user._id, { ...data, avatar, updatedAt: new Date() }, { new: true });
